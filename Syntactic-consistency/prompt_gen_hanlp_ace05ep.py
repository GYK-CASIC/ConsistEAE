...
def arg_identity(event_type):
    """Retrieve the parameters corresponding to the event type"""
    return arg_data.get(event_type, "")


# Load example dataset
with open(f"Syntactic-consistency/ace05ep/pre-extracted-data", 'r') as file:
    data = json.load(file)
...
# Extract dependency relationships from the syntactic tree
...
# Convert dependency tree into a NetworkX graph
def build_tree_graph(dep_tree):
    G = nx.DiGraph()
    for word, dep_rel, head in dep_tree:
        if head != 0:  # Exclude the root node
            parent_word = dep_tree[head - 1][0]  # head is a 1-based index
            G.add_edge(parent_word, word, rel=dep_rel)  # Edge from parent to child
    return G
...
# Get the structural similarity of sentences
...
def find_most_similar_examples_by_structure_random(target_sentence, target_event_type, example_sentences,
                                                   top_n=few_shot, similarity_cache=None):
    filtered_examples = [
    ...
    if not filtered_examples:
        print("No examples of the same event type exist.")
        return []
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in tqdm(futures, desc="Calculating similarities", total=len(futures)):
            ...
            similarities.append((futures[future][0], similarity_score))  # Store sentence index and similarity

    # Sort by similarity score in descending order
    similarities.sort(key=lambda x: x[1], reverse=True)

    # Get the top `top_n` most similar sentence indices
    most_similar_examples = [filtered_examples[idx][0] for idx in range(min(top_n, len(similarities)))]
    print(most_similar_examples)
...
output_prompt_path = f"Syntactic-consistency/ace05ep/prompt_EEA_with_trigger.json"
