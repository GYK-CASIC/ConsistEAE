...
# 一个用于处理AMR格式数据的库
import penman
# 从自定义模块amr_utils中导入了AMR_Alignment等库 
# 分别表示保存和加载对齐信息的json和AMR
from amr_utils.alignments import AMR_Alignment, write_to_json, load_from_json
from amr_utils.amr import AMR
# AMR对象：在 AMR 图（Abstract Meaning Representation）中，AMR 对象是指用于表示句子语义结构的图形数据结构。一个 AMR 对象通常包括以下元素：节点、边、根节点
# 对齐对齐信息指的是 AMR 图中的节点或边与句子中的具体词语之间的映射关系。对齐信息可以告诉我们每个 AMR 图元素是如何对应到原始句子中的单词的。对齐信息在 AMR 图的上下文中尤为重要，因为它使得图中的抽象节点能够与句子中的具体词语关联起来。
# 对齐信息的保存对AMR的解析、应用和再利用非常重要。它可以增强解释性、评估任务表现、支持逆向操作、便于学习和推理



# 用于解析AMR文件的元数据，例如句子ID、标记和对齐信息等。
class Matedata_Parser:
    ...
    def readlines(self, lines):
        # 将元数据中的 `::` 替换为新行，将其解析为元数据字典和图元数据字典。
    ...
    def readline_(self, line):
        # 将单行的元数据解析为键值对，例如“# ::id 123”。
        # 处理元数据行。这个方法返回一个元数据的标签和它的值。
...
# 该类提供了将AMR字符串解析为 AMR 对象的能力，并处理AMR图的节点、边和属性等。
class PENMAN_Wrapper:
    ...
    def parse_amr(self, tokens, amr_string):
        # 使用 `penman` 库解析 AMR 字符串，并将其转换为 AMR 对象。
        # 该方法将节点分为普通节点、属性和边，同时检查是否存在重入节点
...
class AMR_Reader:
    ...
    def load_from_dir(self, dir, remove_wiki=False, output_alignments=False):
        # 从目录加载所有 .txt 文件，逐个文件读取AMR图，并合并到一个列表中。还处理了ID冲突的情况，并为每个文件生成唯一的AMR ID。
        all_amrs = []
        all_alignments = {}
...
def main():
    ...
    # 创建 AMR_Reader 对象，加载目录中的AMR文件并提取对齐信息。
    reader = AMR_Reader()
    # 将AMR图保存到输出文件，并将对齐信息保存到JSON文件。
