...
def generate_edge_tensors(triples, max_seq_length_graph):
    ...
    assert len(position_ids) == max_node + 2 
...
def generate_wiki_edge(graph_triples, max_seq_length):
    ...
    mask_idx = random.sample(range(len(edge_types)), k=int(len(edge_types) * 0.15))  
    masked_edge_index = edge_index.clone()  
    masked_edge_types = edge_types.clone()  
    masked_labels = []
    for idx in mask_idx:
        masked_labels.append(edge_types[idx])
        masked_edge_types[idx] = -1
        
        masked_edge_index[0, idx] = -1 
        masked_edge_index[1, idx] = -1 
    return [masked_edge_index, masked_edge_index], [masked_edge_types, masked_edge_types], [position_ids, position_ids],[mask_idx,mask_idx],[masked_labels,masked_labels]
