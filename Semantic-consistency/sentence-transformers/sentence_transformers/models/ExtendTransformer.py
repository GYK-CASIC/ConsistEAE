...
class ExtendTransformer(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: Optional[int] = None,
                 ...
                 tokenizer_name_or_path: str = None, adapter_size=768, gnn='RGCN', gnn_layer=-1, gnn_head_num=4,
        ...
        self.max_seq_length = max_seq_length or self.auto_model.config.max_position_embeddings

        self.add_gnn = add_gnn
        self.adapter_size = adapter_size
        if self.add_gnn:
            if gnn_layer <= 0:
                raise ValueError("gnn_layer should be a positive integer when add_gnn=True")
            self.gnn_layers = nn.ModuleList([GraphConv(adapter_size, adapter_size) for _ in range(gnn_layer)])
        self.mlm_head = nn.Linear(adapter_size, self.auto_model.config.vocab_size)  
    ...
    def forward(self, features):
        ...
        if edge_index is not None and self.add_gnn:
        ...
        if 'mask_idx' in features:
            masked_idx = features['mask_idx'] 
            masked_labels = features['masked_labels'] 
            batch_size = output_tokens.size(0)
            device = output_tokens.device
            indices = torch.arange(batch_size, device=device)
            masked_embeddings = output_tokens[indices, mask_idx] 

            masked_logits = self.mlm_head(masked_embeddings) 
            features['masked_logits'] = masked_logits
